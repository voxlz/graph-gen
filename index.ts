// index.ts
// Usage: tsx index.ts <input.txt|input.json> <output.png> [style.jsonc]
//
// Pipeline:
...
const graphMeta = {
  ...
  lineColor: spec.graph?.lineColor ?? globalStyle.graph?.lineColor ?? "#000000",
  // clear gap between a boundary's nodes and its border.
  boundaryPad: spec.graph?.boundaryPad ?? globalStyle.graph?.boundaryPad ?? 34,
...
const maxDepth = boundaries.reduce((m, b) => Math.max(m, boundaryDepth(b)), 0);
